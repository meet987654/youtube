...
import { setSearchQuery } from "../utils/searchSlice"; // Import setSearchQuery action
...
const Head = () => {
    ...
    const getSearchSuggestion = async () => {
        const response = await fetch(`${YT_SEARCH_API}${searchInput}`);
    ...
    const handleSearchChange = (e) => {
        ...
        dispatch(setSearchQuery(e.target.value));  // Dispatch the search query to Redux
    };

    return (
        <div className='grid grid-flow-col fixed z-10 bg-white w-full pb-5'>
            ...
            <div className='col-span-10 flex ml-20 text-[18px]'>
                <div>
                    <input
                        className='bg-transparent border border-gray-400 w-[48rem] mt-6 py-[0.55rem] px-2 rounded-l-[30px]'
                        type="text"
                        value={searchInput}
                    ...
                    />
                    <button className='pr-4 pl-3 bg-gray-400 border border-gray-400 h-[46px] absolute mt-6 rounded-r-[30px]'>
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                        <div className='absolute bg-white w-[48rem] rounded-lg p-4 shadow-2xl'>
                            <ul>
                                {suggestions.map((s, index) => (
                                    <li key={index} className='p-1 hover:bg-gray-50 border-t-[1px]'>
                                        {s}
                                    </li> 
                                ))}
                            </ul>
